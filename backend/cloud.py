...
def delete_remote_case(case_id: str) -> bool:
    cid = str(case_id or "").strip()
    if not configured() or not cid:
        return False
    conn = _connect()
    if conn is not None:
        try:
            _ensure(conn)
            with conn.cursor() as cur:
                cur.execute("DELETE FROM earthrelay_cases WHERE id = %s", (cid,))
            conn.commit()
            return True
        except Exception:
            return False
            ...
            conn.close()
    try:
        _rest("DELETE", f"/rest/v1/earthrelay_cases?id=eq.{quote(cid, safe='')}")
        return True
    except Exception:
        return False
...
def _materialize(cid: str, payload, original=None, annotated=None) -> dict:
    try:
        from cases import is_case_deleted

        if is_case_deleted(cid):
            return None
    except Exception:
        pass
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            for cid, payload in rows:
                ...
                if _materialize(cid, payload) is None:
                    continue
                n += 1
...
def _pull_rest() -> int:
    ...
    for row in rows or []:
        ...
        if not cid:
            continue
        if _materialize(cid, row.get("payload") or row) is None:
            continue
        n += 1
    return n
