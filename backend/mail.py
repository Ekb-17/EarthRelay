...
def send_invite(*, to_email: str, role_label: str, join_url: str) -> dict:
    ...
    result = _send(msg)
    if result.get("sent"):
        result["detail"] = "Invitation email sent."
    return result


def send_org_reset(*, to_email: str, code: str, username: str) -> dict:
    if not configured():
        return {"sent": False, "detail": "SMTP is not configured."}
    user = os.getenv("SMTP_USER", "").strip()
    sender = (os.getenv("SMTP_FROM") or user).strip()
    org = (os.getenv("ORG_NAME") or "EarthRelay Response Team").strip()
    msg = EmailMessage()
    msg["Subject"] = f"{org} verification code"
    msg["From"] = sender
    msg["To"] = to_email
    msg.set_content(
        f"Your {org} organization desk verification code is:\n\n"
        f"{code}\n\n"
        f"Username: {username}\n"
    ...
    result = _send(msg)
    if result.get("sent"):
        result["detail"] = "Verification code sent."
    return result


def send_volunteer_reset(*, to_email: str, code: str, name: str = "") -> dict:
    if not configured():
        return {"sent": False, "detail": "SMTP is not configured."}
    user = os.getenv("SMTP_USER", "").strip()
    sender = (os.getenv("SMTP_FROM") or user).strip()
    org = (os.getenv("ORG_NAME") or "EarthRelay Response Team").strip()
    who = (name or "").strip() or "volunteer"
    msg = EmailMessage()
    msg["Subject"] = f"{org} Community Response verification code"
    msg["From"] = sender
    msg["To"] = to_email
    msg.set_content(
        f"Your {org} Community Response verification code is:\n\n"
        f"{code}\n\n"
        f"Account: {who}\n"
        "This code expires in 10 minutes. If you did not request it, ignore this email.\n"
    )
